...
class ActionVaccineAvailable(Action):
    ...
    def name(self):
        return "action_vaccine_available"
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message('The most effective ways to protect yourself and others against COVID-19 are to frequently clean your hands, cover your cough with the bend of elbow or tissue, and maintain a distance of at least 1 meter (3 feet) from people who are coughing or sneezing.')
        return []
...
class ActionTestingCenters(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message(
            'Only Govt. approved Labs & some private Labs are allowed for Testing.Because sample taken ,transportation & storage process have a unique way.')
        dispatcher.utter_message('You can find list of Testing Centers on DashBoard of this app')
        return []
...
class ActionAnimalInfection(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message(
            'TCoronaviruses are a large family of viruses that are common in animals. Occasionally, people get infected with these viruses which may then spread to other people. For example, SARS-CoV was associated with civet cats and MERS-CoV is transmitted by dromedary camels. Possible animal sources of COVID-19 have not yet been confirmed.')
        dispatcher.utter_message('To protect yourself, such as when visiting live animal markets, avoid direct contact with animals and surfaces in contact with animals. Ensure good food safety practices at all times. Handle raw meat, milk or animal organs with care to avoid contamination of uncooked foods and avoid consuming raw or undercooked animal products.')
        return []
...
class ActionDrinkHot(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message(
            'When this virus enter in body .It remain present in Cells. It will not remain only in Throat . It does not die with Hot Water. Hot Water leads to hydration that helps in improving immunity.')
        return []
...
class ActionCleanDoor(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message(
            'If a person having infection of Corona Virus and if he/she cough & touch door knobs than droplets will stay on door knobs for few hours. So if another person who don’t have Corona virus infection touch the door than touch his/her face or eyes than he/she will also infected.')
        dispatcher.utter_message(
...
class ActionDrinkWater(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message(
            'It is not true. But drinking water increase hydration & you feel well. Immunity work well.')
        return []
...
class FirstCase(Action):
    def name(self):
        return "action_first_case"

    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message('The first human cases of COVID-19 were identified in Wuhan City, China in December 2019. At this stage, it is not possible to determine precisely how humans in China were initially infected with SARS-CoV-2.')
        return []
...
class Thanks(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message('I\'m gald i helped you')
        return []


class About(Action):
    def name(self):
        return "action_about_you"

    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message('I\'m a ChatBot designed for this app to help you by providin information related to COVID-19')
        return []
...
class WhatCanDo(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_message('Try asking: \n1. About COVID19\n2. Symptoms of corona')
        return []